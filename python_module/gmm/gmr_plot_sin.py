...
def plot_gmm(mean, covar, plot):
    ...
    ell = mpl.patches.Ellipse(mean, coeff * np.sqrt(v[0]),
                              coeff * np.sqrt(v[1]),
...
if __name__ == '__main__':
    ...
    splot1 = plt.subplot(3, 1, 1)
    plt.scatter(X[:, 0], X[:, 1], .8, color='r')
    plt.scatter(X[:, 2], X[:, 3], .8, color='g')

    clf = mixture.GMM(n_components=3, min_covar=1e-8, covariance_type='full',
              n_iter=100)
    clf.fit(X)
