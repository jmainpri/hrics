...
class MarkerMapper:
    ...
    def assign_marker_names(self, elbow_pads, wrist_pads, r_arm_only):
        ...
        if elbow_pads:
            if r_arm_only:
                ...
                # Get the left arm
                l_arm = sorted(body, key=lambda p: p.y, reverse=False)[0:5]
        ...
        # Using default marker setup
            ...
            if r_arm_only:
                ...
                r_arm = sorted(body, key=lambda p: p.y, reverse=True)[0:7]
                # Remove the right arm from the body list
                body = [ point for point in body if point not in r_arm ]
                ...
                # Get the left arm
                l_arm = sorted(body, key=lambda p: p.y, reverse=False)[0:7]
                # Remove the left arm from the body list
                body = [ point for point in body if point not in l_arm ]
    ...
    def assign_right_arm_names_pads(self, r_arm_points):
        ...
        # Palm has to be first point in the list
        rPalm = r_arm_points[0]
        print "rPalm oid : ", rPalm.original_id
        ...
        # print 'rWristInner : ', rWristInner.original_id
        print "WristOut : ", rWristOuter.original_id, "WristIn : ", rWristInner.original_id
