...
class Segmenter():
    ...
    def __init__(self, m_file, o_file, img_dir, drawer):
        ...
        self.split = [0,0]
        self.splits = []
        # label should start at 1, if 0 means no label
        self.label = [0,0]
        # store the label
        self.labels = []
        # label list, can be modified
        self.label_list = ['up1','up2','up3','up4','down1','down2']
    ...
    def save_splits(self):
        ...
        with open(split_outpath, 'w') as s_file:
    ...
    def save_labels(self):
        #  Get the out filename
        dir, path = os.path.split(self.m_file)
        split_outpath = dir+'/'+'Labels.csv'

        print "saving labels to : ", split_outpath

        with open(split_outpath, 'w') as s_file:
            out_str = str(self.label_list)+'\n'
            s_file.write(out_str)
            for label in self.labels:
                out_str = str(label[0]) + ',' + str(label[1])+','+str(label[2])+','+str(label[3])+'\n'
                s_file.write(out_str)


if __name__ == '__main__':
    base_dir = '/home/ruikun/workspace/aterm_experiment/'
    ...
    for block in blocks:
        ...
        for run in runs:
            ...
            while True:
                c = keystroke.getch(-1)
                if c == 'a':
                    print "Adding label list"
                    print "Current label list:",s.label_list
                    pLabel = raw_input("What label you want to add?")
                    s.label_list.append(pLabel)
                if c == 'f':
                    print "Add FROM label to current segment:"
                    print "Label list:",s.label_list
                    pplabel = input("Choose Label by num index, start from 1...")
                    s.label[0] = pplabel
                if c == 't':
                    print "Add TO label to current segment:"
                    print "Label list:",s.label_list
                    pplabel = input("Choose Label by num index, start from 1...")
                    s.label[1] = pplabel
                ...
                if c == 's':
                    ...
                    s.labels.append((s.split[0], s.split[1],s.label[0],s.label[1]))
                    ...
                    # temp.segment([(s.split[0], s.split[1])])
                if c == 'r':
                    print "Removing split : ", s.splits.pop(), s.labels.pop()
                    continue
                if c == 'x':
                    if len(s.splits) > 0:
                        s.save_splits()
