...
void publishTransforms(const std::string& frame_id) {
    ...
    for (int i = 0; i < users_count; ++i) {
        ...
        publishTransform(user, XN_SKEL_HEAD,           frame_id, "head_2");
        publishTransform(user, XN_SKEL_NECK,           frame_id, "neck_2");
        publishTransform(user, XN_SKEL_TORSO,          frame_id, "torso_2");

        publishTransform(user, XN_SKEL_LEFT_SHOULDER,  frame_id, "left_shoulder_2");
        publishTransform(user, XN_SKEL_LEFT_ELBOW,     frame_id, "left_elbow_2");
        publishTransform(user, XN_SKEL_LEFT_HAND,      frame_id, "left_hand_2");

        publishTransform(user, XN_SKEL_RIGHT_SHOULDER, frame_id, "right_shoulder_2");
        publishTransform(user, XN_SKEL_RIGHT_ELBOW,    frame_id, "right_elbow_2");
        publishTransform(user, XN_SKEL_RIGHT_HAND,     frame_id, "right_hand_2");

        publishTransform(user, XN_SKEL_LEFT_HIP,       frame_id, "left_hip_2");
        publishTransform(user, XN_SKEL_LEFT_KNEE,      frame_id, "left_knee_2");
        publishTransform(user, XN_SKEL_LEFT_FOOT,      frame_id, "left_foot_2");

        publishTransform(user, XN_SKEL_RIGHT_HIP,      frame_id, "right_hip_2");
        publishTransform(user, XN_SKEL_RIGHT_KNEE,     frame_id, "right_knee_2");
        publishTransform(user, XN_SKEL_RIGHT_FOOT,     frame_id, "right_foot_2");
    }
}
...
int main(int argc, char **argv) {
    ...
    string configFilename = ros::package::getPath("openni_tracker_multi") + "/tracker2.xml";
   ...
   // SELECTION OF THE DEVICE
    ...
    printf("The following devices were found:\n");
        ...
        if (argc != 2)
        {
            printf("Choose device to open (2): ");
