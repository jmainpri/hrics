...
class HumanGmmTraining:
    ...
    # Copies the files from the training folder to the gmm_data folder
    # The selected files are the ones that are in the self.class list
    # file ids for each class are defined in the constructor
    #
    # Each class corresponds to a different folder
    #     class_0/
    #     class_1/
    #     ...
    def copy_training_files(self):
    ...
    # Given a trajectory specifying the full DoFs, generates one that only
    # comprises the active DoFs
    # input: traj (has all DoFs)
    # ouput: active_traj (has only the active DoFs)
    ...
    # Goes over the human_one trajectories in the gmm data for all
    ...
    def generate_active_dofs_trajs(self):
        ...
        for id, c in enumerate(self.classes):
            ...
            self.classes_trajs.append(active_dofs_class_trajs)

    # Save active DoFs trajectories to file
    # The motions are re-sampled and saved as a compound file
    # Note: This is only performed for the human_one
    ...
    # Convert the gmr trajectories back to time and full DoFs
    # Regressed trajectories are created by GMM fitting for the class
    # this depends on the settings of the GMM algorithm
    def generate_gmr_trajectories(self):

        for id, c in enumerate(self.classes):

            class_human_one_dir = (
                "gmm_data/classes/human_one/class_" + str(id) + "/")
            class_human_two_dir = (
                "gmm_data/classes/human_two/class_" + str(id) + "/")

            # Create directories for the gmr trajectories
            ...
            # Get the regressed motion for the ith class
            class_gmr_file = (
                "gmm_data/gmr_trajs/traj_class_" + str(id + 1) + ".csv")
    ...
    def generate_gmr_predicted_trajectories(self):

        # HERE CHANGE THE ID and folder
        folder = 'gmm_data/gmr_trajs/'

        classification = np.loadtxt(
            open(folder + '/testing_classification_05.csv', "rb"),
            delimiter=",")
        ...
        gmr_predicted_human_one_dir = folder + "/predicted/"
