...
if __name__ == "__main__":
    ...
    if only_selected:
        selected_splits = list_selected.get_splits_from_file(
            "removed_from_training.txt")
        removed_splits = list_selected.get_splits_from_file(
            "removed_from_testing.txt")
        in_collision_splits = list_selected.get_splits_from_file(
    ...
    for r in runs:
        ...
        for f in files:
            ...
            if not s in splits:
                ...
                if swap:
                    os.rename(type + "/human_one/" + s + "_human2_.csv",
                              type + "/human_one/" + s + "_human1_.csv")
                    os.rename(type + "/human_two/" + s + "_human1_.csv",
                              type + "/human_two/" + s + "_human2_.csv")
